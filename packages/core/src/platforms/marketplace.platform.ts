 ...
 * 
 * Tailors the normalized listing output into clean, structured formats for direct copy-pasting
 * into online secondhand marketplaces (such as eBay, Vinted, Subito, Wallapop, Depop).
 */
...
export class MarketplacePlatform {
  /**
   * Human-friendly label mapping for item condition states.
   */
  ...
  /**
   * Formats the listing result for secondhand marketplaces.
   */
  public static format(result: ListingResult): FormattedListing {
    const conditionLabel =
      MarketplacePlatform.CONDITION_LABELS[result.condition] || result.condition;
...
/**
 * Backward compatibility alias for MarketplacePlatform.
 */
export const VintedPlatform = MarketplacePlatform;
