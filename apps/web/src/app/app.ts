...
import { ListingInput, VintedCondition, ListingResult } from '@vintstack/core';
...
export class App {
  ...
  public exportJson(): void {
    ...
    this.downloadBlob(blob, `vintstack_${this.sanitizeFilename(listing.title)}.json`);
  }
  ...
  public exportMarkdown(): void {
    ...
    const md = [
      ...
      '',
      '---',
      `*Generated by VintStack on ${new Date(listing.createdAt).toLocaleDateString()}*`,
    ].join('\n');

    const blob = new Blob([md], { type: 'text/markdown' });
    this.downloadBlob(blob, `vintstack_${this.sanitizeFilename(listing.title)}.md`);
  }
